...
cd Fish-tank
go build -trimpath -ldflags "-s -w -H windowsgui" -o tank.exe ./cmd/tank
./tank.exe
```
...
**Titan pod (G39–G43, G48–G49).** A five-fish pack of deep-water giants
(`titan-abyssdrifter`, one vast leader + four escorts) lives in the tank
permanently since G59 — born with every fresh world (and restored saves
re-add them on load), never leaving. They are SCALARE (angelfish) like the
reference photo: silver-pearl bodies with dark slate stripes, tall dorsal
and anal fins and trailing ventral streamers. The pod sweeps
...
depth lane, so the small school reads in front of them. Strikes are
live-prey-only since G82: a wriggling treat or water mite triggers a
≥6×-cruise dart (plain flakes never do), and a right-click scare bolts the
whole pod away from the point at the same burst, decaying over the startle
window. Only after starving 20 s straight — with a 300 s tank-wide
cooldown, a population floor and the lilac fish forever immune — may the
leader take one small fish (fast fade, no corpse, one log line). The pod
is not selectable anywhere: catalog filter, SpawnEgg guard, and the
`titan` role is core-seed-only in content validation. It never counts
toward popCap, breeding, aging or snapshots.
...
**Staging & realism (G52, G85, G86).** Giants never linger off the frame — anything
roaming past the margin steers back into view within seconds (a scare bolt
cannot strand one there; the two forces never cancel). The pod cruises a
high band of the water column (roughly the upper third under the surface
light, with a rare shallow dip), and the owner-drawn red line at 0.673H is
cleared at all times — below it lives only the rare strike dive, never the
cruise. Headings stay near horizontal (a level law with hysteresis settles
steep pitches), and crosswise turns bend into the posture clamp instead.
Growth weights the bodies: a bigger pod member cruises heavier but lunges
farther — one tail whip covers real distance — and every tail beat slows
with size.
...
## v1.2 — Movement reality, silk & the REC capture

Three delivered batches, each measured per docs/TRACEABILITY.md rows 107–111
(goals G90–G95).

**Movement reality (G90–G92).** Every fish obeys one continuity law, pinned
by a soak-test governor (`movement_governor_test.go`): head displacement per
frame ≤ carried velocity travel + a bounded reflex + slack, and every spine
joint ≤ a lever/slew/swing allowance — the ±3 px frame hops, the 17 px tail
yanks and the milling flops are closed at their roots (wave-aware bend-cone
slew, layout kink cap, heading-hung trailing axis, eased swim wave, 2 px
rigid edge shift with rope-slide). Big bodies no longer teleport +20/+30 px
at the nest: encounters read as deflections — a lookahead probe steers
traffic around the circle, the drain is depth-graded, and spawns/restores
place residents outside the nest from the first frame.

**The white angels' silk (G94).** The giants' hard triangular fins are
replaced by six white silk filaments — two long mustache barbels off the
snout and four layered veils — riding the simulation's own swim beat: a
slow, heavy half-rate undulation at cruise that quickens only when the pod
strikes at live food. A self-sufficient ceiling law keeps the cruising band
clear without leaning on frame-edge shifts.

**REC 60s capture (G95).** A button beside the X records 60 s of the tank
at 1 frame per second into `screenshots/rec-<timestamp>/` next to the exe:
4:5 portrait frames cropped under the HUD chrome (no UI ever leaks in),
a crop window that pans with the living center of mass of the school,
JPEG q90 (a full session is a few MB), async writes so recording never
costs a frame. Made for the LinkedIn portrait format — frames convert
cleanly into captioned GIFs.

**In development (continuation notes).** The glass-gaze feature — fish that
occasionally swim up to the glass and stare at the viewer, full face-on —
is complete on the `gaze-wip` branch (behavior, renderer, soak tests; row
109) and awaits one documented residual fix (a dense-swarm joint family at
1.0–1.3× the governor allowance) before it rides the release line.
