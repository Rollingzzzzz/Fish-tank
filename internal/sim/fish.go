...
func (f *Fish) advance(dt, night float64, w *World) {
	...
	// G94: the silver elders cruise at ~half the generic big-body rate —
	// the mesmerizing crawl — and the strike burst swings them into a
	// fast flick through the same eased Speed01.
	beatCalm, beatAttack, beatMul := 3.2, 7.5, 1.0
	if f.Sp.Role == contract.RoleTitan {
		beatCalm, beatAttack, beatMul = contract.TitanBeatCalm, contract.TitanBeatAttack, contract.TitanBeatMul
	}
	f.phase += dt * (beatCalm + beatAttack*f.speed01S) * (1 - 0.25*f.ElderP) * phaseMul *
		beatMul * 64 / (64 + contract.BeatBodyDamp*f.bodyLen)
	f.followSpine(dt)
	w.dragSpineOut(f)
