...
func (f *Fish) steerTitan(dt, maxSp float64, w *World) contract.Vec2 {
	...
	// NOTE: the G94 "ceiling twin" force is retired as of the v1.2 release
	// pass. The slow silk beat already keeps the band off the top glass
	// through the G86 altitude law (every band and convoy contract measures
	// green without the twin), and the boundary contact frames the twin used
	// to mask are now correctly owned by the canvas guarantee in the G90
	// governor's contact exemption. Re-introducing any per-member version
	// re-scatters the convoy (measured: p95 390-786 px against the 350
	// contract).
