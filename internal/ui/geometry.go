...
const (
	...
	// REC capture button (G95), top row beside the X.
	RecBtnW = 86
	RecBtnH = 26
)
...
type Metrics struct {
	...
	RecBtn     Rect // G95: always-visible REC 60s capture button
}
...
func UIMetrics(w, h int, open bool) *Metrics {
	...
	m.RecBtn = Rect{X: m.CloseBtn.X - 8 - RecBtnW, Y: 8, W: RecBtnW, H: RecBtnH}
