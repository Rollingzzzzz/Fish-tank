...
type FishAnim struct {
	...
	Beat       float64 // G94: the sim's swim-wave phase — silk undulates with it
}
...
func (b *FishBatch) Draw(dst, glowDst *ebiten.Image, spine []contract.Vec2, spec *contract.Species,
	...
	if spec.Role == contract.RoleTitan {
		// G94: the silver elders wear WHITE SILK, not hard triangles —
		// two snout barbels and four body veils undulating like silk in
		// the sim's own swim beat (slow crawl, fast strike)
		drawTitanSilk(&fins, spine, norms, widths, segs, bodyLen, anim, hide)
	} else {
