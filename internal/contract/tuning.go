...
const (
	...
	MiteDropY            = 54.0  // surface line mites are released from (same lane as the auto-feeder)
	MiteSinkSpeed        = 14.0  // px/s a dropped mite settles downward
	MiteSinkMaxFrac      = 0.72  // depth fraction where the drift stops and the mite lingers
...
const (
	...
	TitanAltMax    = 0.40 // G86: the cruising band stays well above the owner's red line (0.673H)
	TitanAltDip    = 0.52 // G86: the rare dip still clears the red line with body room
	TitanAltDipP   = 0.12 // G84: chance a drawn altitude is the rare dip
	TitanArcFloor  = 0.52 // G86: a down-curl arc never reaches the red line
...
// v1.2 movement reality (G90–G93): the motion governor. No fish, and no
// joint of any fish, may JUMP: single-frame displacement stays inside the
// fish's own travel plus tight reflex slack, and every positional
// correction is a bounded glide. These bind the diagnosis behind the ±3 px
...
const (
	...
	LayoutKinkMax  = 0.8  // rad — the LAYOUT angle between two laid neighbors; what the eye sees
