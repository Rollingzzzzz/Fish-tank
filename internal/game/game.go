...
type Game struct {
	...
	recBtn    ui.ButtonState          // G95: the always-visible REC 60s button
	rec       recorder                // G95: the capture state machine
...
func (g *Game) processInput(mx, my int, pressed, released, rPressed, rReleased bool) {
	...
	um := ui.UIMetrics(ScreenW, ScreenH, g.menu.Open())
	cb, rb := um.CloseBtn, um.RecBtn
	...
	if wasCarrying {
		...
		if g.recBtn.Update(rb.X, rb.Y, rb.W, rb.H, mx, my, pressed, released) {
			g.recToggle() // G95: arm or cancel the REC 60s capture
		}
	...
	if clickEdge && !wasCarrying && g.grabKind == "" &&
		!g.menu.Consumes(mx, my) && !g.tray.Hit(mx, my) &&
		!ui.HitR(cb, mx, my) && !ui.HitR(rb, mx, my) {
	...
	if scareEdge && !g.menu.Consumes(mx, my) && !g.tray.Hit(mx, my) &&
		!ui.HitR(cb, mx, my) && !ui.HitR(rb, mx, my) {
