...
// drawRec paints the REC 60s capture button (G95) beside the X, above the
// menu: red countdown while recording, green flash once frames are saved.
func drawRec(dst *ebiten.Image, b *ui.ButtonState, r *recorder) {
	rm := ui.UIMetrics(ScreenW, ScreenH, false).RecBtn
	label, accent := r.view()
	ui.DrawButton(dst, b, rm.X, rm.Y, rm.W, rm.H, label, accent)
}
